...
impl ChildStatusConfig {
    ...
    /// The expectation is that the child status has been updated before it has
    /// been added to the list of nexus children.
    pub(crate) fn add(
        child_name: &str,
        ...
        if cfg.status.contains_key(child_name) {
            // We don't expect to enter here but just in case, log it and return
            // without updating the child status.
            error!(
